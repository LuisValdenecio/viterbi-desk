  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-[160px]">
        ...
        <DropdownMenuSeparator />
        <DropdownMenuItem asChild>
          <Link className="flex cursor-pointer" href={`?member=${row.original?.user_id}&name=${row.original?.name}`}>
             Quotas
            <DropdownMenuShortcut>
        ...
        {(row.original?.status === 'active') ? (
          ...
          <DropdownMenuItem asChild>
            <Link className="flex cursor-pointer" href={`?reactivate=${row.original?.user_id}&name=${row.original?.name}&role=${row.original?.role}`}>
              Re-activate
              <DropdownMenuShortcut>
