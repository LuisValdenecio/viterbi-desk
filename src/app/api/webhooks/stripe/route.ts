				...
				if (customerDetails?.email) {
					...
					for (const item of lineItems) {
						...
						if (isSubscription) {
							...
							if (priceId === process.env.STRIPE_VITERBI_YEARLY_ENTERPRISE_PLAN_ID!) {
								endDate.setFullYear(endDate.getFullYear() + 1); // 1 year from now
							} else if (priceId === process.env.STRIPE_VITERBI_MONTHLY_ENTERPRISE_PLAN_ID!) {
								endDate.setMonth(endDate.getMonth() + 1); // 1 month from now
							} else {
								throw new Error("Invalid priceId");
							}
							// it is gonna create the subscription if it does not exist already, but if it exists it will update it
							await prisma.stripe_subscription.upsert({
								where: { user_id: user.user_id! },
								create: {
									...
									endDate: endDate,
									plan: "enterprise",
									period: priceId === process.env.STRIPE_VITERBI_YEARLY_ENTERPRISE_PLAN_ID! ? "yearly" : "monthly",
								},
								update: {
									plan: "enterprise",
									period: priceId === process.env.STRIPE_VITERBI_YEARLY_ENTERPRISE_PLAN_ID! ? "yearly" : "monthly",
